...
import { noodleMainMenuRowNoProfile, displayItemName as _displayItemName, renderProfileEmbed, runNoodle } from "./noodle.js";
...
import {
  ...
  getUserTipStats,
  logVisitActivity,
  getVisitPatternSummary as _getVisitPatternSummary,
  createSharedOrder,
...
function cardSpecToTextComponents(card = {}) {
  const raw = card?.toJSON?.() ?? card ?? {};
  const components = [];
  ...
  if (textBlocks.length > 0) {
    components.push({ type: 10, content: textBlocks.join("\n\n") });
  }
  ...
  if (thumbnailUrl) {
    components.push({ type: 12, items: [{ media: { url: thumbnailUrl } }] });
  }
...
function buildRecentPickerPayload(interaction, {
  ...
  return {
    mainComponents: cardSpecToTextComponents(embed),
    notices: [],
    components
  };
}
...
function sharedOrderActionRow(userId, hasActiveOrder, isPartyLeader, canComplete = false) {
  ...
  if (hasActiveOrder) {
    ...
    if (isPartyLeader) {
      ...
      components.push(
        new ButtonBuilder()
          .setCustomId(`noodle-social:action:shared_order_refresh:${userId}`)
...
export function normalizePayloadForReply(interaction, payload = {}) {
  ...
  return convertPayloadToComponentsV2(interaction, sourcePayload);
}
...
async function rawWebhookEditOriginal(interaction, payload) {
  ...
  ensureInteractionClientToken(interaction, "socialRawWebhookEditOriginal");
  try {
    ...
    if (!ensureInteractionClientToken(interaction, "socialRawWebhookEditOriginal:retry")) throw error;
...
async function rawWebhookFollowUp(interaction, payload) {
  ...
  ensureInteractionClientToken(interaction, "socialRawWebhookFollowUp");
  try {
    return await interaction.client.api
      .webhooks(applicationId, token)
    ...
    if (!isWebhookTokenUnavailableError(error)) throw error;
    if (!ensureInteractionClientToken(interaction, "socialRawWebhookFollowUp:retry")) throw error;
    return interaction.client.api
      .webhooks(applicationId, token)
      .post({ data: normalizeRawContainerPayload(payload, { ephemeralFlag: MessageFlags.Ephemeral }) });
  }
...
async function rawChannelEditMessage(interaction, channelId, messageId, payload) {
  ...
  ensureInteractionClientToken(interaction, "socialRawChannelEditMessage");
...
async function handleParty(interaction) {
  ...
  if (lockedResult.ephemeral) {
    const payload = normalizePayloadForReply(interaction, {
...
async function handleComponent(interaction) {
  ...
  if (kind === "select") {
    ...
    if (action === "shared_order_recipe") {
      ...
      const servingsEmbed = buildCardSpec()
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(servingsEmbed),
        notices: [],
    ...
    if (action === "shared_order_confirm") {
      ...
      const lockedResult = await withLock(db, `lock:user:${userId}`, ownerLock, 8000, async () => {
        try {
          ...
          const _result = createSharedOrder(db, party.party_id, recipeId, serverId, servings);
          const viewer = ensurePlayer(serverId, userId);
    ...
    if (action === "shared_order_ingredient") {
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(qtyEmbed),
        notices: [],
  ...
  if (kind === "action") {
    ...
    if (action === "shared_order") {
      ...
      if (embed) {
        ...
        const emptyEmbed = buildCardSpec()
    ...
    if (action === "shared_order_refresh") {
      ...
      if (party.leader_user_id !== userId) {
        return componentCommit(interaction, {
          content: `${getIcon("error")} Only the party leader can refresh the shared order.`,
          ephemeral: true
        });
      }
    ...
    if (action === "shared_order_create") {
      ...
      const createEmbed = buildCardSpec()
        .setTitle(`${getIcon("serve")} Create Shared Order`)
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(createEmbed),
        notices: [],
        components: [new ActionRowBuilder().addComponents(menu), backRow]
      });
    }
    ...
    if (action === "shared_order_contribute") {
      ...
      const isLeader = party.leader_user_id === userId;
      const contributeEmbed = buildCardSpec()
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(contributeEmbed),
        notices: [],
      ...
      const lockedResult = await withLock(db, `lock:user:${userId}`, ownerLock, 8000, async () => {
        try {
          ...
          if (qtyRow && (updatedIngredient?.remaining ?? 0) > 0) {
            const continueEmbed = buildCardSpec()
              .setTitle(`${getIcon("status_complete")} Shared Order • Contribution Recorded`)
              .setDescription(
            ...
            return {
              ok: true,
              payload: {
                mainComponents: cardSpecToTextComponents(continueEmbed),
                notices: [],
                components: [qtyRow, sharedOrderActionRow(userId, true, isLeader, canComplete), socialMainMenuRow(userId)]
              }
            };
    ...
    if (action === "shared_order_complete") {
      ...
      // Confirm completion
      const promptEmbed = buildCardSpec()
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(promptEmbed),
        notices: [],
        components: [
    ...
    if (action === "shared_order_cancel_complete") {
      ...
      const cancelEmbed = buildCardSpec()
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(cancelEmbed),
        notices: [],
        components: [partyActionRow(userId, true, isLeader, !!existingOrder), socialMainMenuRow(userId)]
      });
    }
    ...
    if (action === "shared_order_abort_cancel") {
      ...
      return componentCommit(interaction, {
        mainComponents: cardSpecToTextComponents(keepEmbed),
        notices: [],
